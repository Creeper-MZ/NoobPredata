...
if __name__ == "__main__":
    ...
    tar_files = [f for f in file_list if f.startswith("webp/data-") and f.endswith(".tar")]
    ...
    def collate_fn(batch):
        images = [Image.open(io.BytesIO(sample["webp"])) for sample in batch]
        keys = [sample["__key__"] for sample in batch]
        return images, keys
    num = 0
    batch_size = 2 # 设置batch大小
    dataloader = DataLoader(dataset, batch_size=batch_size, collate_fn=collate_fn)
    for i, (images, keys) in enumerate(dataloader):
        results = stream_chat(images)
